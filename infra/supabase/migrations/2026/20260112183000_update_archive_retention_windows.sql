...
-- ============================================================================
-- Requirement:
-- - Keep published/rejected queue items for 1 year (365 days)
...
BEGIN
  ...
  -- Load status codes from status_lookup (note: 330/approved was removed)
  ...
  WITH moved AS (
    ...
    WHERE (
      ...
      AND status_code IN (v_status_published, v_status_rejected)
    ) OR (
  ...
  -- Step 2: Add published items to seen_urls (NOT rejected, NOT failed)
  ...
  WHERE archived_at >= now() - interval '1 minute'
    AND status_code = v_status_published
