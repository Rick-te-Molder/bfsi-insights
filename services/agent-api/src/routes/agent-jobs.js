...
import { getStepName, handleItemFailure } from '../lib/failure-handling.js';
...
async function processItem(item, agent, config) {
...
// Background processor (refactored to reduce cognitive complexity)
async function processAgentBatch(agent, jobId, items, config) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      await processItem(item, agent, config);
      successCount++;
    } catch (err) {
      console.error(`${agent} failed for ${item.id}:`, err.message);
      failedCount++;
      await handleItemFailure(item, agent, err, config);
    }
  }
