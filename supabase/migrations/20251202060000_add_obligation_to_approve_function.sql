...
BEGIN
  ...
  -- Insert obligation relationships (NEW)
  FOR v_obligation_code IN 
    SELECT jsonb_array_elements_text(COALESCE(v_payload->'obligation_codes', '[]'::jsonb))
  LOOP
    INSERT INTO kb_publication_obligation (publication_id, obligation_code)
...
SELECT 
  ...
  -- Obligations array (NEW)
  COALESCE(
    (SELECT array_agg(po.obligation_code)
     FROM kb_publication_obligation po
     WHERE po.publication_id = p.id),
    '{}'
  ) as obligations,
